...
class _OpenChatroomChatViewState extends State<OpenChatroomChatView> {
  ...
  Future<void> _showUserOptions(BuildContext context, UserModel user) async {
    ...
    try {
      ...
      final exemptDoc = await _firestore.collection('matchExemptions').doc(exemptDocId).get();
      isExempted = exemptDoc.exists;

      final authController = context.read<AuthController>();
      isBlocked = authController.blockedUserIds.contains(user.uid);
    } catch (e) {
      // Ignore
    }
              ...
              children: [
                if (isOwner && isTargetInChatroom)
                  ListTile(
                    ...
                    onTap: () {
                      ...
                      _showRemoveFromChatroomDialog(user);
                    },
                  ),
                ListTile(
                ...
                ListTile(
                  ...
                  onTap: () {
                    Navigator.pop(context);
                    if (isExempted) {
                      _showUnexemptDialog(user);
                    } else {
                      _showExemptFromMatchingDialog(user);
                    }
                  },
                ),
                ListTile(
  ...
  void _showRemoveFromChatroomDialog(UserModel user) {
    showDialog(
      context: context,
...
''';
  ...
  void _showExemptFromMatchingDialog(UserModel user) {
    final l10n = AppLocalizations.of(context)!;
    final currentUser = context.read<AuthController>().currentUserModel;
    if (currentUser == null) return;
          ...
          ElevatedButton(
            onPressed: () async {
              try {
                ...
                await _firestore.collection('matchExemptions').doc(exemptDocId).set({
  ...
  void _showUnexemptDialog(UserModel user) {
    final l10n = AppLocalizations.of(context)!;
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
          ...
          ElevatedButton(
            onPressed: () async {
              try {
                ...
                await _firestore.collection('matchExemptions').doc(exemptDocId).delete();
          ...
          ElevatedButton(
            onPressed: () async {
              try {
                ...
                await _firestore.collection('blocks').doc(blockDocId).set({
                ...
                // Update local blocked list in AuthController
                await authController.refreshCurrentUser();
